...
import jp.co.comster.web.form.ActorForm;
...
import jp.co.comster.web.repository.ActorRepository;
import jp.co.comster.web.repository.PrefectureRepository;
...
public class ActorController {
  ...
  public String index(Model model) {
    ...
    List<ActorWithPref> list = actorRepository.selectAllWithPref();
  ...
  public ModelAndView detail(@PathVariable Integer id) {
    ...
    ActorWithPref actor = actorRepository.selectByIdWithPref(id);
  ...
  public ModelAndView search(@RequestParam String keyword) {
    ...
    if (StringUtils.isNotEmpty(keyword)) {
      List<ActorWithPref> list = actorRepository.findActors(keyword);
      if (CollectionUtils.isEmpty(list)) {
        String message = msg.getMessage("actor.list.empty", null, Locale.JAPAN);
  ...
  public String save(@Validated @ModelAttribute ActorForm form, BindingResult result, Model model) {
    ...
    return "redirect:/actor/" + actor.id.toString();
  }
  ...
  public String delete(@PathVariable Integer id, RedirectAttributes attributes, Model model) {
    ...
    Actor delActor = actorRepository.selectById(id);
  ...
  private Actor convert(ActorForm form) {
    ...
    actor.name = form.getName();
    if (StringUtils.isNotEmpty(form.getHeight())) {
      actor.height = Integer.valueOf(form.getHeight());
    }
    if (StringUtils.isNotEmpty(form.getBlood())) {
      actor.blood = form.getBlood();
    }
    if (StringUtils.isNotEmpty(form.getBirthday())) {
      ...
      actor.birthday = Date.from(instant);
    }
    if (StringUtils.isNotEmpty(form.getBirthplaceId())) {
      actor.birthplaceId = Integer.valueOf(form.getBirthplaceId());
    }
    actor.updateAt = new Date();
    return actor;
  }
